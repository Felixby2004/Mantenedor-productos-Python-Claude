...
def _kpi_table(kpis: List[Dict], styles, total_width=17.5*cm) -> Table:
    """Crear tabla de KPIs visuales en fila, ajustada al ancho disponible."""
    header_row = [Paragraph(k["label"], styles["KPILabel"]) for k in kpis]
    value_row  = [Paragraph(k["value"], styles["KPIValue"]) for k in kpis]

    n_kpis = len(kpis)
    col_width = total_width / n_kpis

    tabla = Table(
        [header_row, value_row],
        colWidths=[col_width] * n_kpis,
...
def generar_reporte_inventario(
    ...
    # Ancho para página horizontal (landscape A4: 29.7cm - márgenes)
    elementos.append(_kpi_table(kpis, styles, total_width=26.5*cm))
...
def generar_reporte_analisis(
    ...
    # Ancho para página vertical (A4: 21cm - márgenes)
    elementos.append(_kpi_table(kpis, styles, total_width=17.5*cm))
    ...
    cat_widths  = [4.5 * cm, 2.5 * cm, 2.5 * cm, 4 * cm, 2.5 * cm]
    ...
    if not bajo_stock:
        ...
        bs_headers = ["SKU", "Producto", "Categoría", "S. Act.", "S. Mín.", "Déf.", "Proveedor"]
        bs_widths  = [2.2*cm, 4.5*cm, 2.5*cm, 1.8*cm, 1.8*cm, 1.5*cm, 3.5*cm]
